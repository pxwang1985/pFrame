...
EventDispatcher.initialize = function (target) {
	...
	target.termEventListener = eventDispatcher.termEventListener;
};
...
EventDispatcher.prototype.termEventListener = function(){
	var repo = this[EventDispatcher.REFERENCE_NAME].listenerRepository;
	for (var Key in repo){
		repo[Key].splice(0,repo[Key].length);
		delete(repo[Key])
	}	
	
	delete(this[EventDispatcher.REFERENCE_NAME].listenerRepository);
	delete(this[EventDispatcher.REFERENCE_NAME]);
	delete(this.dispatchEvent);
	delete(this.addEventListener);
	delete(this.removeEventListener);
	delete(this.termEventListener);
}